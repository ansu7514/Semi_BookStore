...
public class OrderDAO {
	...
	//전체주문조회 (페이징)
	public ArrayList<OrderDTO> orderList(String user_id, int start, int perpage){
		Connection conn = db.getConnection();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	 ...
	 * 
	 * //바인딩 pstmt.setInt(1, start); pstmt.setInt(2, perpage);
	 * 
	 * //실행 rs=pstmt.executeQuery();
	 * 
	 * while(rs.next()) { OrderDTO dto=new OrderDTO();
	 * 
	 * dto.setOrder_id(rs.getString("order_id"));
	 * dto.setUser_id(rs.getString("user_id"));
	 * dto.setBook_id(rs.getString("book_id"));
	 ...
	 * dto.setLocation(rs.getString("location"));
	 * dto.setBook_price(Integer.parseInt(rs.getString("book_price")));
	 * dto.setPayDay(rs.getDate("pay_day")); dto.setEa(rs.getInt("ea"));
	 * dto.setPay_method(rs.getString("pay_method"));
	 * 
	 * //리스트에 추가 list.add(dto); }
	 * 
	 * } catch (SQLException e) {
	 * 
